...
local inc = -1
local timer = nil
local color = string.format("#%x", vim.api.nvim_get_hl(0, { name = "@alpha.title", link = false }).fg or 16777215)
local background = string.format("#%x", vim.api.nvim_get_hl(0, { name = "Normal", link = false }).bg or 16777215)
...
M.update_colors = function()
  ...
  background = string.format("#%x", vim.api.nvim_get_hl(0, { name = "Normal", link = false }).bg or 0)
  color = string.format("#%x", vim.api.nvim_get_hl(0, { name = "@alpha.title", link = false }).fg or 0)
...
M.in_mathzone = function(_, matched_trigger)
  ...
  while ancestor_node do
    ...
    elseif ancestor_node:type() == "generic_command" then
      local cmd_name = vim.treesitter.get_node_text(assert(ancestor_node:named_child(0)), 0, {})
